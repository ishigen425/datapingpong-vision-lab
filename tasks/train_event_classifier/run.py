...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Subset of trained non-none classes to emit and evaluate, e.g. --prediction-events bounce.",
    )
    ...
    prediction_events = args.prediction_events or [name for name in args.classes if name != "none"]
    validate_prediction_events(prediction_events, args.classes)
    ...
    reference = [row for row in events if row["item"] in set(args.test_items) and row["event"] in set(prediction_events)]
    summary = summarize(reference, predictions, tolerance=args.tolerance, event_names=prediction_events)
    summary.update(
        {
            "ball": str(args.ball),
...
def validate_prediction_events(prediction_events: list[str], classes: list[str]) -> None:
    invalid = set(prediction_events) - (set(classes) - {"none"})
    if invalid:
        raise ValueError(f"--prediction-events must be trained non-none classes. Invalid: {sorted(invalid)}")
